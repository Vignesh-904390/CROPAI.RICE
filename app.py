...
DISEASE_JSON_PATH = 'rice_disease_info.json'
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Rice Crop Disease Detection",
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🌾 New Rice Crop Disease Detection Result",
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['cropai2025@gmail.com'])
        
        # Create email body with prediction results
        email_body = f"""
        🔍 Rice Crop Disease Detection Result
...
def send_email():
    ...
    if not (name and email and msgt):
        ...
        return redirect('/')
    try:
        m = Message("🌾 New Contact Request - Rice Crop Disease Detection", sender=app.config['MAIL_USERNAME'], recipients=['tdaitech@gmail.com'])
        m.body = f"Name:{name}\nEmail:{email}\nMessage:{msgt}"
        ...
        r.body = f"Hi {name},\nWe received your message regarding Rice crop disease detection."
        mail.send(r)
