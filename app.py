...
MODEL_PATH = 'model/tomato_effnet.pth'           # <-- rice model path
DISEASE_JSON_PATH = 'rice_disease_info.json'   # <-- rice disease info JSON
...
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(f"Model file not found at {MODEL_PATH}. Please place your .pth checkpoint there.")
...
# Use new torchvision weights API to avoid warnings; weights=None is equivalent to pretrained=False
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Rice Disease Detection",
